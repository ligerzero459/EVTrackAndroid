...
public class RecentAdapter extends ArrayAdapter<BattledPokemon> implements Filterable {
	
	private ArrayList<BattledPokemon> original;
	private ArrayList<BattledPokemon> filtered;
	private Activity mContext;

	public RecentAdapter(ArrayList<BattledPokemon> battled, Activity context)
	{
		super(context, 0, battled);
		original = new ArrayList<BattledPokemon>(battled);
		filtered = new ArrayList<BattledPokemon>(battled);
		mContext = context;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		if (convertView == null)
		{
			convertView = mContext.getLayoutInflater().inflate(R.layout.list_item_recent, null);
		}

		Resources resource = mContext.getResources();
		BattledPokemon p = getItem(position);
		...
		TextView timesBattled = (TextView)convertView.findViewById(R.id.times_battled);
		timesBattled.setText("" + p.getBattled());
	...
	Filter filter = new Filter() {
		...
		protected void publishResults(CharSequence constraint, FilterResults results) {
			filtered = (ArrayList<BattledPokemon>)results.values;
			notifyDataSetChanged();
			clear();
			for (BattledPokemon p : filtered)
			{
				add(p);
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			{
				ArrayList<BattledPokemon> tempList = new ArrayList<BattledPokemon>();
				for (BattledPokemon p : original)
				{
			...
			{
				ArrayList<BattledPokemon> tempList = new ArrayList<BattledPokemon>(original);
				filterResults.values = tempList;
				filterResults.count = tempList.size();
			}
